...
def setup_event_writer(is_wandb_enabled: bool, is_tensorboard_enabled: bool, log_dir: Path, wandb_config: Optional[cfg.WandbConfig] = None) -> None:
    ...
    if is_wandb_enabled:
        curr_writer = WandbWriter(log_dir=log_dir, wandb_config=wandb_config)
        EVENT_WRITERS.append(curr_writer)
...
class WandbWriter(Writer):
    ...
    def __init__(self, log_dir: Path, wandb_config: Optional[cfg.WandbConfig] = None):
        if wandb_config:
            wandb.init(id=wandb_config.run_id, dir=str(log_dir))
        else:
            wandb.init(reinit=True, dir=str(log_dir))
